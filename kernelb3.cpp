...
static double Omega_M(double a, double OmegaM0){
	...
	int ode2(double a, const double D[], double dDda[], void *params)
  {
  	...
  	double p3     = p.par3;
		...
		double D1k1 = calc_D1(k1, k_arr, D1a_arr);
		double D1k2 = calc_D1(k2, k_arr, D1a_arr);

		//std::cout << D1_arr[a_num*Nk + k1_num] << "  " << D1_arr[a_num*Nk + k2_num] << '\n';
		//std::cout << "D1(k1):" << D1k1 << "  D1(k2):" << D1k2 << '\n';

		/*
		double D11_old = round_to_digits(D1_arr[a_num*Nk + k1_num], 10);//round(D1_arr[a_num*Nk + k1_num]);//
		double D12_old = round_to_digits(D1_arr[a_num*Nk + k2_num], 10);//round(D1_arr[a_num*Nk + k2_num]);//
		double D11_new = round_to_digits(D1k1, 10);//round(D1k1);//
		double D12_new = round_to_digits(D1k2, 10);//round(D1k2);//

		std::cout.precision(99);

		if(abs(D11_old) != abs(D11_new));{
		...
		*/
		...
		dDda[1] = - ( (3./a) + (dHA(a, OmegaM0)/HA(a, OmegaM0)) )*D[1] + (3./2.)*(OmegaM0/(pow2(a)*(OmegaM0 + OmegaL0*pow3(a))))*mu(a, k, OmegaM0, p1, p2, p3)*D[0]; + (3./2.)*(OmegaM0/(pow2(a)*(OmegaM0 + OmegaL0*pow3(a))))*mu(a, k, OmegaM0, p1, p2, p3)*D1k1*D1k2*(1-pow2(k1dotk2)) + ( 2./(pow4(a)*pow2(HA(a, OmegaM0))) )*gamma2(a, OmegaM0, k, k1, k2, 1., p1, p2, p3); //u1 not needed for f(R), need to FIX for DGP
  ...
  //#pragma omp parallel for schedule(dynamic)
		for(int k1_num = 0; k1_num < Nk; k1_num++){
			...
			for(int x_num = 0; x_num < Nx; x_num++){
  			...
  			/*Parameters passed to system of equations */
  			// EDIT : If more than one gravity parameter is used, add them after p1
  			struct param_type4 my_params2 = {k, k1, k2, k1dotk2, OmegaM0, par1, par2, par3, A_num};// k1_num, k2_num};//
...
int main(int argc, char* argv[]) {
	...
	for(int a_num = 0 ; a_num <Na; a_num++){
		...
		for(int k_num = 0 ; k_num < Nk;  k_num ++){
				...
				solve2(a_num, a_val, k, omega0, 0.0001, 1., 1.);
				...
				for(int k1_num = 0; k1_num < Nk; k1_num++){
					...
					for(int x_num = 0; x_num < Nx; x_num++){
					  ...
					  //p3 = F2A_nk[j*n2+m];
						...
						p4 = kL2[k1_num*Nx+x_num]/(1.-pow2(k1dotk2));
						p5 = -7.*p4/(3.*D1_arr[a_num*Nk + k_num]);
